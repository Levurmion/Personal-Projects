...
import { SLRClosureSet } from "./closure-set";
import { SLRItem } from "./item";
import type { SLRGotoSet, SLRDFAState } from "./types";

export class SLRAutomaton<
...
> {
    ...
    public readonly DFAStates: Map<number, SLRDFAState>;
    ...
    constructor(language: Language<GTokens, GNonTerminals>) {
        ...
        if (language.grammar.startSymbol !== AUGMENTED_START) {
            throw new Error(`Cannot construct an SLRItemSet without an augmented grammar.`);
        }
    ...
    public CLOSURE(item: SLRItem): SLRClosureSet {
        const closureSet = new SLRClosureSet();

        const queue = [item];
        while (queue.length > 0) {
            // proccess the currItem
            const currItem = queue.pop() as SLRItem;
            ...
            if (
                ...
                const nextItems = adjNonTerminalProductions.map(
                    (productionRule) => new SLRItem(productionRule),
                );
    ...
    public GOTO(stateId: number, symbol: string): SLRGotoSet | null {
        let items = new SLRClosureSet();
        const coreItems = new SLRClosureSet();
     ...
     */
    private getStateIdWithCoreItems(coreItems: SLRClosureSet): number | null {
        const coreItemsSignature = coreItems.getSignature();
    ...
    private buildDFA() {
        ...
        let nextStateId = 1;
        const queue: [number, SLRDFAState][] = [[0, this.DFAStates.get(0) as SLRDFAState]];
        while (queue.length > 0) {
            const [stateId, state] = queue.pop() as [number, SLRDFAState];
